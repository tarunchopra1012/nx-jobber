...
The generated client is written into the app itself, at `apps/jobber-auth/src/app/prisma/generated` (set by the `output` field of the `client` generator), so that each service owns its own client without collisions. It is gitignored and regenerated by the `generate-types` target, which `build` depends on.
